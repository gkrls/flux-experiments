...
def color_of(k):
    ...
    if k.startswith("nccl-rdma"): return "#2E5645" if "ring" in k else "#5B8373"
    if k.startswith("dpa"):       return {"dpa-64": "#8BADC7", "dpa-128": "#5A8DB5", "dpa-256": "#2D6A9F"}.get(k.split("/")[0], "#5A8DB5")
    return "#888"
...
lat      = data["allreduce"]["latency"]
lat_sz   = data["allreduce-size"]["latency"]
...
def plot_throughput(ax, size, title):
    ...
    for k in THROUGHPUT_BARS:
        runs = data["allreduce"]["throughput"].get(k, {}).get("data", {}).get(size, [])
    ...
    for i, k in enumerate(keys):
        for ovl_key, ls in OVERLAYS.get(k, []):
            runs = data["allreduce"]["throughput"].get(ovl_key, {}).get("data", {}).get(size, [])
            if not runs: continue
            m, _ = avg_gbit(runs)
...
ymax_tp = 0
ymax_tp = max(ymax_tp, plot_throughput(axes[0,0], "100", "Throughput: 100MB tensors (20-batch)"))
ymax_tp = max(ymax_tp, plot_throughput(axes[0,1], "25", "Throughput: 25MB tensors (20-batch)"))
for a in axes[0,:2]: a.set_ylim(0, ymax_tp)

# -- throughput scaling vs tensor size --
ax = axes[0, 2]
tp_line_sty = line_styles(THROUGHPUT_LINE)
for k in THROUGHPUT_LINE:
    sty = tp_line_sty[k]
    sizes_mb, gbits, errs = [], [], []
    for sz_str, run in sorted(data["allreduce-size"]["throughput"].get(k, {}).items(), key=lambda x: float(x[0])):
...
def plot_latency(ax, size, title):
    ...
    for k in LATENCY_BARS:
        run = data["allreduce"]["latency"].get(k, {}).get("data", {}).get(size)
        if not run: continue
        m, s = avg_latency(run)
    ...
    for i, k in enumerate(keys):
        for ovl_key, ls in OVERLAYS.get(k, []):
            run = data["allreduce"]["latency"].get(ovl_key, {}).get("data", {}).get(size)
            if not run: continue
            m, _ = avg_latency(run)
            if m is None: continue
            ax.hlines(m, x[i]-W/2, x[i]+W/2, ls=ls, colors='#8B1A1A', lw=2, zorder=5)
...
ymax_lat = 0
ymax_lat = max(ymax_lat, plot_latency(axes[1,0], "100", "Latency: 100MB tensors (20x)"))
ymax_lat = max(ymax_lat, plot_latency(axes[1,1], "25", "Latency: 25MB tensors (20x)"))
for a in axes[1,:2]: a.set_ylim(0, ymax_lat)

# -- latency scaling vs tensor size --
ax = axes[1, 2]
has_data = False
...
plt.savefig("allreduce_perf.pdf", format='pdf', bbox_inches='tight')
plt.show()