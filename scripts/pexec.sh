...
# Usage: ./pexec.sh [--keep] session host1 host2 ... -S script [args]
#    or: ./pexec.sh [--keep] session host1 host2 ... -- command [args]
#    or: ./pexec.sh [--keep] session -f hostfile -S script [args]
#    or: ./pexec.sh [--keep] session -f hostfile -- command [args]
...
for HOST in "${HOSTS[@]}"; do
    ...
    if [[ "$MODE" == "-S" ]]; then
        ...
        if [[ "$KEEP_OPEN" == "true" ]]; then
            ssh "$HOST" "tmux new-session -d -s '$SESSION' 'export WORLD_SIZE=$WORLD_SIZE RANK=$i; bash -l $REMOTE_SCRIPT $ARGS; rm -f $REMOTE_SCRIPT; exec bash'"
        else
            ssh "$HOST" "tmux new-session -d -s '$SESSION' 'export WORLD_SIZE=$WORLD_SIZE RANK=$i; bash -l $REMOTE_SCRIPT $ARGS; rm -f $REMOTE_SCRIPT'"
        fi
    else
        # -- mode
        COMMAND="$*"
        if [[ "$KEEP_OPEN" == "true" ]]; then
            ssh "$HOST" "tmux new-session -d -s '$SESSION' 'export WORLD_SIZE=$WORLD_SIZE RANK=$i; bash -l -c \"$COMMAND\"; exec bash'"
        else
            ssh "$HOST" "tmux new-session -d -s '$SESSION' 'export WORLD_SIZE=$WORLD_SIZE RANK=$i; bash -l -c \"$COMMAND\"'"
        fi
    fi
