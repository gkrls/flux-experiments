...
# SCRIPT=${0##*/}
echo "[resnet/${0##*/}] iface=$IFACE ip=$IP rank=$RANK world_size=$WORLD_SIZE master=${MASTER_ADDR}:${MASTER_PORT} backend=$BACKEND"

export DPA_PREEMPTIVE=0
export DPA_TORCH_MODE=worksteal

# echo "====== Resnet training ======"
# sudo -E DPA_LOG=INFO DPA_SCHEDULER=OFF $(which python) experiments/train/resnet-train.py \
#   --rank "$RANK" \
#   --world_size "$WORLD" \
#   --iface "$IFACE" \
...
#   --prefetch_factor 4 \
#   --drop_last_val \
#   --json experiments/train/resnet.json \
#   --dpa_k 6 # or 5 for SA
  # --straggle_points 3 \
  # --straggle_prob 15 \
  # --straggle_ranks 1 \
...
echo "====== Resnet step timing ======"
sudo -E DPA_LOG=INFO DPA_SCHEDULER=ON $(which python) experiments/train/resnet-step.py \
  --rank "$RANK" \
  --world_size "$WORLD" \
  --iface "$IFACE" \
  --master_addr "$MASTER_ADDR" \
  ...
  --prefetch_factor 4 \
  --drop_last_val \
  --json experiments/train/resnet-step.json \
  --log_every_steps 50 \
  --dpa_k 6
